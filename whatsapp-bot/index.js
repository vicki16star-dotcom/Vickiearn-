...
const client = new Client({
  authStrategy: new LocalAuth({ clientId: 'group-moderator' }),
  puppeteer: {
    headless: true,
...
client.on('ready', () => {
  botReady = true;
  latestQr = null;
  console.log('WhatsApp moderation bot is READY.');
});
...
Moderation commands are admin-only.`);
      return;
    }
    ...
    if (text.startsWith('!warn')) {
      ...
      if (count >= 3) await safeReply(message, `🚨 @${user.number} has reached 3 warnings. An admin should review this member.`, [user]);
      return;
    }
...
app.get('/', (_req, res) => {
  res.send(`<h2>WhatsApp Moderator</h2><p>Status: ${botReady ? 'ONLINE' : 'WAITING FOR WHATSAPP LINK'}</p><p><a href="/qr">Open QR code</a></p>`);
});
